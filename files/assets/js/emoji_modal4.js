        ...
        for (i=0; i < emojis.length; i++) {
            ...
            for (j=0; j < arr.length; j++) {
                if(arr[j].match(search_bar.value)){
                    str += `<button class="btn m-1 px-0 emoji" onclick="getEmoji(\'${arr[j]}\', \'${form}\')" style="background: None!important; width:60px; overflow: hidden; border: none;" data-bs-toggle="tooltip" title=":${arr[j]}:" delay:="0"><img loading="lazy" width=50 src="/assets/images/emojis2/${arr[j]}.webp" alt="${arr[j]}-emoji"/></button>`;
                }
            }
