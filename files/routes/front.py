...
def notifications(v):
	...
	if messages:
		if v.admin_level == 6: comments = g.db.query(Comment).filter(Comment.child_comments is None, or_(Comment.author_id==v.id, Comment.sentto==v.id, Comment.sentto==0)).filter(Comment.parent_submission == None).order_by(Comment.created_utc.desc()).offset(25 * (page - 1)).limit(26).all()
